...
const AttachEventSourceMutation_1 = require("../graphQL/AttachEventSourceMutation");
const helpers_1 = require("../helpers");
compass_requests_1.CompassRequests.prototype.attachEventSource = async function (input) {
    let gqlErrors, mutationErrors, errorsResp = [];
    let data;
    try {
        const resp = await this.api.requestGraph(AttachEventSourceMutation_1.AttachEventSource, { input });
        ({ errors: gqlErrors, data } = await resp.json());
        errorsResp = errorsResp.concat((0, helpers_1.mapGqlErrors)(gqlErrors));
        ({ attachEventSource: { errors: mutationErrors } } = data.compass);
...
//# sourceMappingURL=attachEventSource.js.map