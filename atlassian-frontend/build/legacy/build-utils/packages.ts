...
import git from './git';
import { AFPackageJson, AFPackageWithRelativeDir } from './types';
...
export async function getTransitiveDependencies(
  ...
  const depGraph = await bolt.getDependencyGraph({ cwd, excludedTypes });
...
export async function getChangedPackagesWithMandatoryChangesetSinceCommit(
  ...
  const changedFiles = (await git.getChangedFilesSince(commit, true)).filter(
    (changedFile: string) => {
      ...
      const isInSrcFileRegex = new RegExp('^.*/src/.*.(ts|js)x?$');
