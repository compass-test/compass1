...
const defaultOpts: Pick<
  ...
  'killOthers' | 'prefix' | 'raw' | 'outputStream' | 'kill'
> = {
  ...
  raw: false,
  // Provide our own kill handler that overrides the default concurrently one as the default
  // one uses `ps` which breaks in CI since it is not installed in our docker image
  kill: (pid: number, code?: string) => {
    try {
      process.kill(Number(pid), code);
    } catch (e) {
      // ESRCH means it couldnt find the pid to kill, possibly because it already exited
      if (e.code !== 'ESRCH') {
        throw e;
      }
    }
