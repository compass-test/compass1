...
const noChangesetMessage = `<div style="border: 2px solid red; padding: 10px; border-radius: 10px; display: inline-block;">
  ...
  <p>See <a href="https://developer.atlassian.com/cloud/framework/atlassian-frontend/development/07-versioning/#releasing-packages-1" target="_parent">this guide</a> for more details.</p>
</div>`;
...
const releasedPackagesMessage = (releases, shouldWarnAboutChangesetsV2) => {
  ...
  const dependentsWarningSection = shouldWarnAboutChangesetsV2
    ? `<p style="color: red;">Warning: Dependents can not currently be displayed by the Release Addon when using Changesets V2.<br>
         ...
         <p style="color: rgb(80, 95, 121)">For any questions, please see the <strong>#atlassian-frontend</strong> room.</p>`
    : '';
...
function displayChangesetMessageOnAddOn(msg) {
  document.querySelector('.actual-content').innerHTML = msg;
}
...
async function main() {
  ...
  if (releases.length === 0) {
    if (changesetsPresent) {
      displayChangesetMessageOnAddOn(noReleaseEmtpyChangesetMessage);
    } else {
      displayChangesetMessageOnAddOn(noChangesetMessage);
    }
    return;
  }
  displayChangesetMessageOnAddOn(
    releasedPackagesMessage(releases, shouldWarnAboutChangesetsV2),
  );
}
...
try {
  ...
  displayChangesetMessageOnAddOn(errorLoadingChangesetMessage);
}
