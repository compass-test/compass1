...
export const ModalSidebar = styled.div`
  ...
  flex-shrink: 0;
  flex-grow: 0;
  border-right: 2px ${colors.N30} solid;
...
export const IgnoredIcon = styled.div`
  height: ${3 * gridSize()}px;
  width: ${3 * gridSize()}px;
  position: relative;
  &::before {
    ...
    background: ${colors.N20};
    border-radius: 100%;
    margin: -${1.25 * gridSize()}px 0 0 -${1.25 * gridSize()}px;
    top: 50%;
    left: 50%;
  }
`;
...
export const AnnotationExpander = styled.div<{ isExpanded: boolean }>`
  width: ${2 * gridSize()}px;
  height: ${2 * gridSize()}px;
  ...
  justify-content: center;
  margin: 0 auto;
  ${({ isExpanded }) => isExpanded && expandedStyles}
`;

export const PrToggleWrapper = styled.div`
  display: flex;
  align-items: center;
  strong {
    color: ${colors.N500};
...
export const prToggleTheme = (currentTheme: any, themeProps: any) => {
  const { buttonStyles, ...rest } = currentTheme(themeProps);
  return {
    buttonStyles: { ...buttonStyles, borderRadius: `${2 * gridSize()}px` },
    ...rest,
  };
};
