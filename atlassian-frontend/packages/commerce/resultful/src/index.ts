...
export type FailureResult<E, EX = any | Error> =
  | ErrorResult<E>
  | ExceptionResult<EX>;
...
export const isSuccessful = <P>(
  result: Result<P, unknown>,
): result is SuccessResult<P> => result.payload !== undefined;

export const isFailure = <E, EX = any | Error>(
  result: Result<unknown, E, EX>,
): result is FailureResult<E, EX> => result.payload === undefined;

export const createErrorResult = <E>(error: E): ErrorResult<E> => ({
  error,
  payload: undefined,
  exception: undefined,
});

export const createSuccessResult = <T>(payload: T): SuccessResult<T> => ({
  payload,
  error: undefined,
  exception: undefined,
});

export const createExceptionResult = <E extends Error = Error>(
  exception: E,
): ExceptionResult<E> => {
  return {
    payload: undefined,
