...
import { LibrariesEmptyState } from '@atlassian/dragonfruit-common-ui/assets';
import {
  getSearchQuery,
...
import { LibrariesPageLoad } from '../../utils/performance-analytics';

import messages from './messages';

const DEFAULT_FILTERS: Array<TeamFilter> = [
  { name: 'type', filter: { eq: CompassComponentType.LIBRARY } },
];

const LibrariesView: React.FC = () => {
  ...
  const emptyState = (
    <EmptyState
      ...
      imageUrl={LibrariesEmptyState}
      primaryAction={
        <CreateComponentButton type={CompassComponentType.LIBRARY} />
      }
    />
  );
  ...
  return (
    <>
      ...
      {!loading && <SubmitPageLoadMetrics metric={LibrariesPageLoad} />}
    </>
  );
};

const LibrariesViewWithContext: React.FC = () => {
  return (
    <SelectedFiltersProvider>
      <LibrariesView />
    </SelectedFiltersProvider>
...
export default withErrorBoundary(LibrariesViewWithContext, {
  Fallback: ListViewErrorBoundaryFallback,
  componentName: 'librariesList',
});
