...
import { OtherEmptyState } from '@atlassian/dragonfruit-common-ui/assets';
import {
  getSearchQuery,
...
import { OtherComponentsPageLoad } from '../../utils/performance-analytics';

import messages from './messages';

const DEFAULT_FILTERS: Array<TeamFilter> = [
  { name: 'type', filter: { eq: CompassComponentType.OTHER } },
];
...
const OtherView: React.FC = () => {
  ...
  const emptyState = (
    <EmptyState
      ...
      imageUrl={OtherEmptyState}
      primaryAction={
        <CreateComponentButton type={CompassComponentType.OTHER} />
      }
    />
  );
  ...
  return (
    <>
      ...
      {!loading && <SubmitPageLoadMetrics metric={OtherComponentsPageLoad} />}
    </>
  );
};

const OtherViewWithContext: React.FC = () => {
  return (
    <SelectedFiltersProvider>
      <OtherView />
    </SelectedFiltersProvider>
...
export default withErrorBoundary(OtherViewWithContext, {
  Fallback: ListViewErrorBoundaryFallback,
  componentName: 'otherComponentsList',
});
