...
import { ServicesEmptyState } from '@atlassian/dragonfruit-common-ui/assets';
import {
  getSearchQuery,
...
import { ServicesPageLoad } from '../../utils/performance-analytics';

import messages from './messages';

const DEFAULT_FILTERS: Array<TeamFilter> = [
  { name: 'type', filter: { eq: CompassComponentType.SERVICE } },
];
...
const ServicesView: React.FC = () => {
  ...
  const emptyState = (
    <EmptyState
      ...
      imageUrl={ServicesEmptyState}
      primaryAction={
        <CreateComponentButton type={CompassComponentType.SERVICE} />
      }
    />
  );
  ...
  return (
    <>
      ...
      {!loading && <SubmitPageLoadMetrics metric={ServicesPageLoad} />}
    </>
  );
};

const ServicesViewWithContext: React.FC = () => {
  return (
    <SelectedFiltersProvider>
      <ServicesView />
    </SelectedFiltersProvider>
...
export default withErrorBoundary(ServicesViewWithContext, {
