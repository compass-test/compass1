...
describe('utils/with-cached', () => {
  ...
  it('should call original function twice if previous promise is resolved', async () => {
    ...
    jest.advanceTimersByTime(DEFAULT_RELEASE_RESOLVED_PROMISE_DELAY);
   ...
   * Rejections
   */
  it('should not keep cached value if promise is rejected on second call', async () => {
    expect.assertions(3);
  ...
  it('should not cached value if promise is rejected on second call when passing a custom timeout', async () => {
    expect.assertions(3);

    const customTimeout = 10000;
    const fn = jest.fn((a) => Promise.resolve(a));
    const wrappedFn = withCached(fn, customTimeout);
    const a0 = { a: 0 };
  ...
  describe('reset', () => {
    it('should reset caches when result of fn is a Promise', async () => {
      expect.assertions(4);
