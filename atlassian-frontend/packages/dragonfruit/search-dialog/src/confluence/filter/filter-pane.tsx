...
import { getPeopleFilterOptions } from './filter-pane-populate-options';
...
export const FilterPane: FunctionComponent<FilterPaneProps> = ({
  ...
  const { searchClient, collabGraphClient } = useClients();
  const {
    peopleFilters: {
  ...
  const hasFilters = availablePeopleFilters.length > 0;
  ...
  return (
    <>
      ...
      <SpaceFilter isLoading={isLoading} />
      {!isAnonymousUser && (
