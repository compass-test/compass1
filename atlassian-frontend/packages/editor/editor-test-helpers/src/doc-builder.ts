...
export function nodeFactory(type: NodeType, attrs = {}, marks?: Mark[]) {
  return function (...content: BuilderContent[]): DocBuilder {
    return (schema) => {
...
export const caption = nodeFactory(sampleSchema.nodes.caption, {});
...
export const panelNote = panel({ panelType: PanelType.NOTE });
...
export const media = (attrs: MediaADFAttrs) =>
...
export const embedCard = (attrs: EmbedCardAttributes) =>
  nodeFactory(sampleSchema.nodes.embedCard, attrs);
...
export const unsupportedMark = (attrs: { originalValue: any }) =>
  markFactory(sampleSchema.marks.unsupportedMark, attrs);
export const unsupportedNodeAttribute = (attrs: {
  type: any;
  unsupported: any;
}) => markFactory(sampleSchema.marks.unsupportedNodeAttribute, attrs);
...
export const dataConsumer = (attrs: DataConsumerAttributes) =>
  markFactory(sampleSchema.marks.dataConsumer, attrs);
