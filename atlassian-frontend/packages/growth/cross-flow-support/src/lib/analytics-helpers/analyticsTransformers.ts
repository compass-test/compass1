...
import { GasV3Payload, AnalyticsContext, AnalyticsPayload } from './types';
...
const getSources = (context: AnalyticsContext) =>
  extractFromEventContext(['source'], context);

const getExtraAttributes = (context: AnalyticsContext) =>
...
const getPackageInfo = (context: AnalyticsContext) =>
  context
    .map((contextItem) => ({
