...
function getPageDetailMockData(idPrefix: any, id: any, noOfUsers: any) {
  const modifiedTimes = [
  ...
  const pageDetailsMock = {
    ...
    history: {
      lastUpdated: {
        ...
        friendlyWhen: modifiedTimes[id % modifiedTimes.length],
      },
      contributors: {
        publishers: {
          users: [] as any[],
  ...
  const userCountBasedContributors: any[] = [
    null,
    [Alvin],
    [Arjun, Angie],
  ...
  for (let i = 0; i < noOfUsers; i++) {
    pageDetailsMock.history.contributors.publishers.users.push({
      ...
      profilePicture: {
        path: userCountBasedContributors[noOfUsers][i],
      },
...
function createPageDetailsMockResponse(size: number, { idPrefix = '' } = {}) {
  const results: any[] = [];
...
function createPageListMockResponse(size: number, { idPrefix = '' } = {}) {
  const results: any[] = [];
  const titles = [
  ...
  for (let i = 0; i < size; i++) {
    const details = detailsMock.results.find(
      (d) => d.id === `${idPrefix}-${i + 1}`,
    );
    results.push({
      ...details,
