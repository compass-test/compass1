...
export default class FlagValue extends React.Component<
...
> {
  ...
  getFlagValue = (client?: FeatureFlagClient): void => {
    if (client) {
      const { argsText } = this.state;
      const args = JSON.parse(argsText);
      const flagValue = client.getFlagValue(
        args.flagKey,
        args.defaultValue,
        args.options,
      );
  ...
  renderControls = (client?: FeatureFlagClient): ReactNode => {
    ...
    return (
      <Button
        appearance="primary"
        isDisabled={disabled}
        onClick={(): void => this.getFlagValue(client)}
      >
        Get Flag Value
      </Button>
    );
      ...
      <Grid layout="fluid">
        ...
        <GridColumn medium={6}>
          ...
          <GridContent>{`${JSON.stringify(
            flagValue,
          )}: type of ${typeof flagValue}`}</GridContent>
        </GridColumn>
      </Grid>
    );
  }
