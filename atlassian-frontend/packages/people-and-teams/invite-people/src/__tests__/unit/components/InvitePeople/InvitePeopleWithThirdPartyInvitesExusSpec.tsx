...
describe('Third Party Invites', () => {
  beforeEach(() => {
    (exusThirdPartyInvitesClient.getExistingIntegrations as jest.Mock).mockResolvedValue(
      {
        connectedIntegrations: [],
        enabledIntegrations: [SLACK_SERVICE, GOOGLE_SERVICE, MICROSOFT_SERVICE],
      },
    );
    (exusThirdPartyInvitesClient.fetchSlackWorkspaces as jest.Mock).mockResolvedValue(
      [],
    );
  ...
  it('should roll out if there is an error and report exposed event with error', async () => {
    (exusThirdPartyInvitesClient.getExistingIntegrations as jest.Mock).mockRejectedValue(
      new Error('error'),
    );
    renderInvitePeople(defaultProps);
    await waitForFormToLoad();
  ...
  it('should call exus api to fetch integrations and slack workspaces', async () => {
    renderInvitePeople(defaultProps);
    await waitForFormToLoad();

    expect(exusThirdPartyInvitesClient.getExistingIntegrations).toBeCalledTimes(
      1,
    );
    expect(exusThirdPartyInvitesClient.fetchSlackWorkspaces).toBeCalledTimes(1);
  });
  ...
  it('should not show any connect prompts if all integerations are set up', async () => {
    ...
    (exusThirdPartyInvitesClient.fetchSlackUsers as jest.Mock).mockResolvedValue(
      [],
    );
