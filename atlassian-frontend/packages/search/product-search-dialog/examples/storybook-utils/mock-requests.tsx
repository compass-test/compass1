...
import {
  RecentPageResponse,
  RecentSpaceResponse,
} from '../../src/confluence/clients/confluence-recents-client';
...
export const mockRecents = () => {
  ...
  const recentSpacesPath = `rest/recentlyviewed/1.0/recent/spaces`;

  // Mock the recent call path
  utils.intercept<RecentPageResponse[]>(
    (_: ServiceConfig, options?: RequestServiceOptions) => {
  ...
  utils.intercept<RecentSpaceResponse[]>(
    (_: ServiceConfig, options?: RequestServiceOptions) => {
      if (options && options.path && options.path === recentSpacesPath) {
        return delayedPromise(createRecentSpace(3), delay());
      }
