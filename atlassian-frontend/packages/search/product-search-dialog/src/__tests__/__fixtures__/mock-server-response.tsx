...
export function createRecentSpace(
  numberOfResults: number,
): RecentSpaceResponse[] {
  return array(numberOfResults).map(() => ({
    id: faker.random.uuid(),
    key: `KEY-${faker.random.word().toUpperCase()}`,
...
export function createRecentPage(
  ...
  overrides: Partial<RecentPageResponse> = {},
): RecentPageResponse {
  const defaultPage = {
    available: true,
    contentType: type,
    id: faker.random.number({ min: 0, max: 100000, precision: 1 }),
    lastSeen: faker.random.number({ min: 0, max: 1000, precision: 0 }),
    space: `Search & Smarts ${faker.random.uuid()}`,
    spaceKey: `abc ${faker.random.uuid()}`,
    title: `Page title ${faker.random.uuid()}`,
    type: `page ${faker.random.uuid()}`,
    url: `/content/123 ${faker.random.uuid()}`,
    iconClass: `iconClass${faker.random.uuid()}`,
  };
