...
export class ConfluenceSearchClient {
  ...
  constructor(
    ...
    this.scopes = {
      ...
      userScope: scopes?.userScope
        ? (scopes.userScope as enumUserScope)
        : Scope.People,
    };
  }
  ...
  public search(
    ...
    const responseAndTiming = this.aggregatorClient.search({
      ...
      context,
      scopes: [
        this.scopes.itemScope,
        this.scopes.spaceScope,
        this.scopes.userScope,
      ],
      modelParams,
      resultLimit: ConfluenceSearchClient.ITEM_RESULT_LIMIT,
      filters: [],
    ...
    const allResults = {
      [Scope.ConfluencePageBlogAttachment]: CancellablePromise.from(
        responseAndTiming.then(({ response, requestDurationMs }) =>
          this.mapConfItem(
            response.retrieveScope(this.scopes.itemScope),
            requestDurationMs,
          ),
        ),
      ),
      [Scope.ConfluenceSpace]: CancellablePromise.from(
        responseAndTiming.then(({ response, requestDurationMs }) =>
          this.mapConfSpace(
            response.retrieveScope(this.scopes.spaceScope),
            requestDurationMs,
          ),
        ),
      ),
      [Scope.People]: CancellablePromise.from(
        responseAndTiming.then(({ response, requestDurationMs }) =>
          this.mapCpusPeople(
            response.retrieveScope(this.scopes.userScope),
            requestDurationMs,
            sites.length > 0,
          ),
        ),
