...
const getApplicableRestrictions = (restrictions, basePath, currentFilepath) =>
  restrictions
    .map(option => {
      const from =
        option.from == null || option.from instanceof Array
          ? option.from
          : [option.from];
      const target =
        option.target instanceof Array ? option.target : [option.target];
...
module.exports = {
  meta: {
    ...
    messages: {
      invalidImport:
        "Importing '{{from}}' within '{{target}}' is restricted{{message}}",
      provideConfig:
  ...
  create(context) {
    ...
    if (
      ...
      configOptions.restrictions.length === 0
    ) {
      context.report({
