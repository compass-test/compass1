...
const filesMatch = (files, struct) => {
  ...
  const result = files.reduce(
    ...
    {
      required: fileRequirements.required,
      optional: fileRequirements.optional,
      extra: {},
    },
  );
  ...
  const missingRequiredFiles = !Object.values(result.required).every(
    i => i.found === true,
  );
  if (missingRequiredFiles) {
    const missing = Object.entries(result.required).reduce(
      (acc, [key, value]) => {
        if (!value.found) {
          acc[key] = value;
          delete value.found;
...
const resolveDefinitionAlias = (alias, definitionsByName) => {
  ...
  while (result.type && result.type !== FILE && result.type !== DIR) {
    ...
    if (visited[result.type]) {
      throw new Error(
        `Definition for ${alias.type} contains a circular reference`,
      );
    }
...
const matchHierarchy = (
  ...
  const resolvedDefinition = resolveDefinitionAlias(
    definition,
    definitionsByName,
  );
...
const dirsMatch = (
  ...
  const result = dirs.reduce(
      ...
      if (currentRequired && !currentRequired.found) {
        if (currentRequired.type === DIR) {
          ...
          const match = matchHierarchy(
            ...
            currentRequired.type
              ? definitionsByName[currentRequired.type]
              : currentRequired,
            definitionsByName,
          );
        ...
        if (currentOptional.type === DIR) {
          ...
          const match = matchHierarchy(
            path.join(currentPath, i),
            childrenByDir,
            currentOptional.type
              ? definitionsByName[currentOptional.type]
              : currentOptional,
            definitionsByName,
          );
    ...
    {
      required: dirRequirements.required,
      optional: dirRequirements.optional,
      extra: {},
    },
  );
  ...
  const missingRequiredDirs = !Object.values(result.required).every(
    i => i.found === true,
  );
  ...
  const mismatchingOptionalDirs = Object.values(result.optional).some(
    i => i.found === false,
  );
  if (mismatchingOptionalDirs) {
    const missing = Object.entries(result.optional).reduce(
      (acc, [key, value]) => {
...
const getDirObject = root => {
  ...
  while (stack.length) {
    ...
    children.forEach(i => {
      // We want to exclude node_modules from the tree.
      if (i === 'node_modules') {
        return;
      }
...
const prepareStruct = struct => {
  const convertValue = valueToConvert => {
    ...
    if (typeof valueToConvert.type === 'undefined') {
  ...
  const result = Object.entries(struct).reduce((agg, [key, value]) => {
    if (key.includes(path.sep)) {
      ...
      agg[key] = existingValue
        ? mergeValues(convertedValue, existingValue, key)
        : convertedValue;
    }
...
const processMismatches = (match, prefix = '.') => {
  ...
  if (match.files) {
    if (match.files.missing) {
      result.push(
        ...Object.keys(match.files.missing).map(
          i => `Missing file ${prefix}${sep}${i}`,
        ),
      );
    }
    ...
    if (match.files.extra) {
      result.push(
        ...Object.keys(match.files.extra).map(
          i => `Could not match file ${prefix}${sep}${i}`,
        ),
      );
    }
  }
      ...
      result.push(
        ...Object.entries(match.dirs.missing).reduce((acc, [dir, mismatch]) => {
          ...
          const processedMismatches = mismatch.matches.reduce(
            (innerAcc, innerMatch) =>
              innerAcc.concat(
                processMismatches(innerMatch, `${prefix}${sep}${dir}`),
              ),
            [],
          );
...
module.exports = {
  ...
  onProject: ({ config, rootPath }) => {
    const definitions = prepareStruct(config.definitions);
    const errorTemplate = config.errorTemplate || '#messages#';

    const pkgRootPath = config.rootPath
      ? path.resolve(rootPath, config.rootPath)
      : rootPath;
    const childrenByDir = getDirObject(pkgRootPath);

    const matchResult = performMatch(pkgRootPath, childrenByDir, definitions);
    const mismatches = processMismatches(matchResult, pkgRootPath);

    return mismatches.map(error =>
      `${errorTemplate}`.replace('#messages#', error),
    );
  },
  meta: {
    docs: {
