...
describe('integration', () => {
  test('use stricter rootPath as root directory if config.rootPath does not exist', () => {
    const first = onProject({
      config: {
        definitions: {
          '.': {
      ...
      rootPath: resolve(__dirname, '__fixtures__', 'single-package-project'),
    });
    expect(first).toEqual([]);

    const second = onProject({
      config: {
        definitions: {
          '.': {
            'package.json': { type: 'file' },
      ...
      rootPath: resolve(
        __dirname,
        '__fixtures__',
        'bad-single-package-project',
      ),
    });
    expect(second).toEqual([
      expect.stringMatching(
        new RegExp(
          `Missing file .*${sep}__fixtures__${sep}bad-single-package-project${sep}package.json`,
        ),
      ),
    ]);
  });
  test('use config.rootPath as root directory if it exists', () => {
    const first = onProject({
      config: {
        definitions: {
          '.': {
        ...
        rootPath: 'good-package',
      },
      rootPath: resolve(
        __dirname,
        '__fixtures__',
        'multi-packages-project',
        'src',
      ),
    });
    expect(first).toEqual([]);
    ...
    expect(second).toEqual([
      expect.stringMatching(
        new RegExp(
          `Missing file .*${sep}__fixtures__${sep}multi-packages-project${sep}src${sep}missing-package-json${sep}package.json`,
        ),
      ),
    ]);
  });
...
describe('onProject', () => {
  test('single package with node_modules should exclude node_modules but node_modules dir should be present', () => {
    const first = onProject({
      config: {
        definitions: {
          '.': {
      ...
      rootPath: resolve(
        __dirname,
        '__fixtures__',
        'single-package-with-node-modules',
      ),
    });
    expect(first).toEqual([
      expect.stringMatching(
        new RegExp(
          `Missing directory .*${sep}__fixtures__${sep}single-package-with-node-modules${sep}node_modules`,
        ),
      ),
    ]);
    const isNodeModulesFolderExists = fs.existsSync(
      resolve(
        __dirname,
        '__fixtures__',
        'single-package-with-node-modules',
    ...
    expect(isNodeModulesFolderExists).toBeTruthy();
  });
  test('multi packages with node_modules should exclude node_modules but node_modules dir should be present', () => {
    const first = onProject({
      config: {
        definitions: {
          '.': {
      ...
      rootPath: resolve(
        __dirname,
        '__fixtures__',
        'multi-packages-with-node-modules',
        'src',
      ),
    });
    expect(first).toEqual([
      expect.stringMatching(
        new RegExp(
          `Missing directory .*${sep}__fixtures__${sep}multi-packages-with-node-modules${sep}src${sep}good-package${sep}node_modules`,
        ),
      ),
    ]);
    const isGoodPkgNodeModulesFolderExists = fs.existsSync(
      resolve(
        __dirname,
        '__fixtures__',
        'multi-packages-with-node-modules',
    ...
    expect(isGoodPkgNodeModulesFolderExists).toBeTruthy();
    const second = onProject({
      config: {
        definitions: {
          '.': {
    ...
    expect(second).toEqual([
      expect.stringMatching(
        new RegExp(
          `Missing directory .*${sep}__fixtures__${sep}multi-packages-with-node-modules${sep}src${sep}good-package_1${sep}node_modules`,
        ),
      ),
    ]);
    const isGoodPkg1NodeModulesFolderExists = fs.existsSync(
      resolve(
        __dirname,
        '__fixtures__',
        'multi-packages-with-node-modules',
    ...
    expect(isGoodPkg1NodeModulesFolderExists).toBeTruthy();
  });
});
...
describe('filesMatch', () => {
  ...
  test('files match corresponding struct with optional', () => {
...
describe('dirsMatch', () => {
  ...
  test('dirs match corresponding struct with optional', () => {
    const result = dirsMatch(
      '',
      ['test1'],
      {},
...
describe('matchHierarchy', () => {
  test('can match multiple dir levels', () => {
    const result = matchHierarchy(
      ...
      {
        level1: { level2: { type: 'level2' } },
        level2: { level3: { type: 'dir' } },
      },
    );
...
describe('prepareStruct', () => {
  test('expands type names', () => {
    ...
    expect(result).toEqual({
      ...
      prepared: { type: 'prepared-type', optional: true },
      '*': [
        { type: 'dir', optional: false },
        { type: 'file', optional: false },
      ],
    });
  });
...
describe('resolveDefinitionAlias', () => {
  test('stops at dir', () => {
    ...
    const result = resolveDefinitionAlias(
      definitionsByName.app,
      definitionsByName,
    );
  ...
  test('throws an error if no type is found', () => {
    ...
    expect(() =>
      resolveDefinitionAlias(definitionsByName.app, definitionsByName),
    ).toThrow(/not found/);
  });
