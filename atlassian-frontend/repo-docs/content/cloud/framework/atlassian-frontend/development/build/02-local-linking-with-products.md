...
# Locally linking in Atlassian Frontend with products
...
Before you start linking you need to have the desired product repo checked out and setup on your machine. If you have not done this, go [here](https://hello.atlassian.net/wiki/spaces/AFP/pages/908938554/Local+linking+with+product+repos) to find the product repo you are after and how to set it up.

## Linking

Linking packages in Atlassian Frontend is more difficult than the standard workflow of running the built-in `yarn link` command because of it. This is because of the majority of packages in the repo need to be built as `yarn link` uses symlinks and building all packages in the repo takes a significant amount of time.
...
You can run the `yarn clean:build` command to delete built package artifacts such as dist folders and entry point directories.
...
This is a known issue related to the fact that we are building a single package in isolation. Other atlassian-frontend package types are not built and so typescript reports 'module not found' errors. These just result in types being coerced to any where used, so shouldn't cause a problem in the target repo. See the caveat in same caveats as [individual package builds][individual-package-builds]
...
The easiest approach would be linking the peer dependency from atlassian-frontend to the target repo or vice versa.
...
[individual-package-builds]: https://developer.atlassian.com/cloud/framework/atlassian-frontend/development/build/04-individual-package-builds
