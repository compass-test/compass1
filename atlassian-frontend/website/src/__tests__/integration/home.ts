...
const packages = '[href="/packages"]';
const allPackages = 'h1[data-testid="all-packages"]';

BrowserTestCase(
  'home.js: The main links on the website should be clikable',
  {},
  async (client: WebdriverIO.BrowserObject) => {
    ...
    await homeTest.click(packages);
    await homeTest.waitForSelector(allPackages);

    const pageTitleText = await client.getTitle();

    expect(pageTitleText).toContain('Browse all packages');
  },
);
