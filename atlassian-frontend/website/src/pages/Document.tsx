...
export default function Document({
  ...
  if (!docId) {
    ...
    while (repoDocs.id !== 'atlassian-frontend') {
      const childDir = repoDocs.children.find(group => group.type === 'dir');
      repoDocs = childDir;
    }

    const found = fs.getFiles(repoDocs.children)[0];
  ...
  const filePath = `repo-docs/content/cloud/framework/atlassian-frontend/${docId}`;
